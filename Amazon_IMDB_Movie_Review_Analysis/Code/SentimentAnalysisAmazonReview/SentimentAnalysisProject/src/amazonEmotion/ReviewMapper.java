...
public  class ReviewMapper extends Mapper<LongWritable, Text, DoubleWritable,DoubleArrayWritable > {
	...
	// static HashSet<String> trustWords;			// 7
	...
	protected void map(LongWritable fileOffset, Text lineContents, Context context) throws IOException, InterruptedException {
		String inputLine= lineContents.toString().trim();
		if(inputLine!=null ){
			if(inputLine.toLowerCase().contains("product/productId".toLowerCase())){
				line=line+lineContents.toString();
			}else{
		...
		int index=line.indexOf("product/productId");
		if(index>0 && index<line.length()){
		line = line.substring(index);
		}
		if(inputLine.toLowerCase().contains("review/text".toLowerCase())){
			flag= true;
		}
		if(flag){
		AmazonRecord record= new AmazonRecord(line.trim());
		flag=false;
		//System.out.println("outside if");
		if(record.getProduct_productId()!=null && record.getReview_text()!=null){
			line = "";
			String review = record.getReview_text();
			
			// Tokenize, sentence split, POS tag and lemmatize
			LinkedList<String> words = new LinkedList<String>();
			Annotation document = new Annotation(review.replace(" " , "_"));
			...
			//	System.out.println("theScores.."+theScores[0]);
			//System.out.println("record.getReview_score().."+record.getReview_score());
			context.write(new DoubleWritable(record.getReview_score()), as);
		}
		}
