...
public class Job5Driver extends Configured implements Tool {

	private static Logger THE_LOGGER = Logger.getLogger(Job5Driver.class);

	@Override
	public int run(String[] args) throws Exception {
		Job job = new Job(getConf());
		job.setJarByClass(Job5Driver.class);
		job.setJobName("Job1");
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length < 3) {
			THE_LOGGER.warn("usage Job1Driver <input1> <input2> <output>");
			System.exit(1);
		}

		THE_LOGGER.info("inputDir=" + args[0]);
		THE_LOGGER.info("outputDir=" + args[1]);
		int returnStatus = ToolRunner.run(new Job5Driver(), args);
