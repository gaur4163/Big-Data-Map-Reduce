	...
	public void reduce(Text key, Iterable<Text> values, Context context)
		...
		for (int i = 0; i < moviesList.size(); i++) {
			for (int j = i + 1; j < moviesList.size(); j++) {
				...
				K2.set(movie1Tokens[0].trim() + "," + movie2Tokens[0].trim()+":");
				V2.set(movie1Tokens[1].trim() + "," + movie2Tokens[1].trim());
				//System.out.println("keys are "+ K2.toString());
				if(!movie1Tokens[0].trim().equalsIgnoreCase(movie2Tokens[0].trim())) context.write(K2, V2);
			}
		}
	}
}